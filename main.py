...
def add_data():
    try:
        potentiometer = request.json.get("potentiometer")
        ...
        add_values= document_ref.document().create(dict(potentiometer= potentiometer, date= datetime.datetime.utcnow() ))
...
def index():
   ...
   try:
    ...
    return render_template("/home/home.html", data=data)
        ...
        print(str(e)) 
        return jsonify({ "status": "error", "message": "No data in database yet!" }), 400
...
if __name__  ==  "__main__":
    app.run(host='192.168.29.67', port=5000)
